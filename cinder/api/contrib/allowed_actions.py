...
#
#
class VolumeAllowedOperationsController(wsgi.Controller):
    def __init__(self, *args, **kwargs):
        super(VolumeActionsController, self).__init__(*args, **kwargs)
        self.volume_api = volume.API()

    @wsgi.action('os-allowed-operations')
    def _attach(self, req, id, body):
        """Add attachment metadata."""
        import pdb;pdb.set_trace()
        context = req.environ['cinder.context']
        rules = policy.Enforcer()
        print rules
        try:
...
class Volume_allowed_operations(extensions.ExtensionDescriptor):
    """Allowed API calls for the current user."""

    name = "VolumeAllowedOperations"
    alias = "os-volume-allowed-operations"
    namespace = "http://docs.openstack.org/volume/ext/volume-allowed-operations/api/v1.1"
    updated = "2014-11-20T00:00:00+00:00"

    def get_controller_extensions(self):
        controller = VolumeAllowedOperationsController()
        extension = extensions.ControllerExtension(self, 'allowedoperations', controller)
        return [extension]

    def get_resources(self):
        resources = []
        res = extensions.ResourceExtension('allowed_operations',
                VolumeTypeExtraSpecsController(),
                parent=dict(member_name='operations',
