...
from cinder.share.drivers import netapp_cmode
...
class NetAppShareDriverTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self._driver = netapp_cmode.NetAppClusteredShareDriver(self._db)
        self._driver._client = self.mox.CreateMock(netapp_cmode.NetAppApiClient)
        cifs_helper = self.mox.CreateMock(netapp_cmode.NetAppCIFSHelper)
        nfs_helper = self.mox.CreateMock(netapp_cmode.NetAppNFSHelper)
    ...
    def test_load_balancer(self):
        ...
        drv._client.get_available_aggregates().AndReturn(
                                                fake_aggr(max_aggr_id))

        self.mox.ReplayAll()

        aggr_id = drv._find_best_aggregate()

        self.assertEquals(max_aggr_id, aggr_id)

    def test_allocate_container(self):
        drv = self._driver
        client = drv._client
        share = {'id': 'fakeshareid', 'size': 1}
        max_aggr_id = 'cmode_filer'

        client.get_available_aggregates().AndReturn(
                                                fake_aggr(max_aggr_id))
        client.send_request_to('volume-create', IgnoreArg())
    ...
    def test_allocate_container_from_snapshot(self):
        ...
        client.send_request_to('volume-clone-create', IgnoreArg())
    ...
    def test_deallocate_container_target_exists(self):
        ...
        client.get_available_aggregates().AndReturn(
            fake_aggr(max_aggr_id))
        client.send_request_to('volume-create', IgnoreArg())
        client.send_request_to('volume-offline', IgnoreArg())
        client.send_request_to('volume-destroy', IgnoreArg())
    ...
    def test_create_snapshot(self):
        ...
        client.send_request_to('snapshot-create', IgnoreArg())
    ...
    def test_delete_snapshot(self):
        ...
        client.send_request_to('snapshot-list-info', IgnoreArg(),
                               do_response_check=False).\
            AndReturn(FakeSnapshots(netapp_cmode._get_valid_snapshot_name(
                snapshot['id'])))
        client.send_request_to('snapshot-delete', IgnoreArg())
...
class NetAppNfsHelperTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        fake_client = self.mox.CreateMock(netapp_cmode.NetAppApiClient)
        self._driver = netapp_cmode.NetAppNFSHelper(fake_client)
...
class NetAppCifsHelperTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        fake_client = self.mox.CreateMock(netapp_cmode.NetAppApiClient)
        self._driver = netapp_cmode.NetAppCIFSHelper(fake_client)
...
class NetAppNASHelperTestCase(test.TestCase):
    def setUp(self):
        ...
        self._driver = netapp_cmode.NetAppNASHelperBase(fake_client)
...
class NetAppApiClientTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self._context = context.get_admin_context()
        self._driver = netapp_cmode.NetAppApiClient()
    ...
    def test_successfull_setup(self):
        ...
        for flag in drv.REQUIRED_FLAGS:
            setattr(netapp_cmode.FLAGS, flag, 'val')
